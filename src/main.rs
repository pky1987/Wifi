...
use windows::{
    ...
    Win32::{
        ...
        NetworkManagement::WiFi::{
            WlanCloseHandle, WlanEnumInterfaces, WlanFreeMemory, WlanGetProfile, WlanGetProfileList, WlanOpenHandle,
            WLAN_API_VERSION_2_0, WLAN_INTERFACE_INFO_LIST, WLAN_PROFILE_GET_PLAINTEXT_KEY, WLAN_PROFILE_INFO_LIST,
        },
    },
};
...
fn grab_interface_profiles(handle: HANDLE, interface_guid: &GUID) -> Result<*mut WLAN_PROFILE_INFO_LIST, windows::core::Error> {
    ...
    'node_traverse: for node in node_path {
        ...
        for subtree_value in &subtree_list {
            ...
            if element_name.to_os_string() == node_name {
                if element_name.to_string().to_lowercase() == last_node_name.to_lowercase() {
...
fn get_profile_xml(handle: HANDLE, interface_guid: &GUID, profile_name: &OsString) -> Result<OsString, windows::core::Error> {
    ...
    let xml_string = match unsafe { profile_xml_data.to_hstring() } {
        Ok(data) => data,
        Err(e) => {
            unsafe { WlanFreeMemory(profile_xml_data.as_ptr() as *mut _) };
            return Err(e);
        }
    };

    unsafe { WlanFreeMemory(profile_xml_data.as_ptr() as *mut _) };
    Ok(OsString::from(xml_string.to_string_lossy()))
}
    ...
    for interface_info in interfaces_list {
        ...
        for profile in wlan_profile_list {
            ...
            let xml_document = match load_xml_data(&profile_xml_data) {
                Ok(xml) => xml,
                Err(_e) => {
                    eprintln!("Failed to extract XML documents");
                    continue;
                }
            };
            ...
            match auth_type.as_str() {
                ...
                "WPA2" | "WPA2PSK" | "WPA3SAE" => {
                    if let Some(password) = traverse_xml_tree(&root, &["MSM", "security", "sharedKey", "keyMaterial"]) {
                        println!("WiFi-Name: {}, Authentication: {}, Password: {}", profile_name.to_string_lossy().to_string(), auth_type, password);
                    }
                }
                _ => {
                    println!("WiFi-Name: {}, Authentication: {}", profile_name.to_string_lossy().to_string(), auth_type);
                }
            }
        }

        unsafe { WlanFreeMemory(wlan_profile_ptr as *mut _) };
    }

    unsafe { WlanFreeMemory(interface_ptr as *mut _) };
    unsafe { WlanCloseHandle(wlan_handle, None) };
}
